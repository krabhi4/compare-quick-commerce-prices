...
class InstamartScraper(BaseScraper):
    ...
    def _parse_item(self, item_data: dict) -> PlatformProduct | None:
        try:
            name = item_data.get("display_name") or item_data.get("name")
            if not name:
                return None
            ...
            if isinstance(offer_price_info, dict):
                price = offer_price_info.get("units") or offer_price_info.get("amount")
    ...
    async def search(
        ...
        async with self.lock:
            ...
            try:
                ...
                if not products:
                    cards = await page.query_selector_all("[data-testid='ItemCard']")
                    for card in cards:
                        name_elem = await card.query_selector("[data-testid='ItemName']")
                        price_elem = await card.query_selector("[data-testid='ItemPrice']")
